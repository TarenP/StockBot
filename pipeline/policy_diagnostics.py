...
def raw_policy_diagnostics(logits: torch.Tensor, temperature: float = 1.0) -> dict:
    logits_np = logits.detach().float().cpu().numpy().reshape(-1)
    concentration = F.softplus(logits.detach().float() / temperature) + 1e-6
...
def action_transform_trace(logits: torch.Tensor, final_weights: Iterable[float] | np.ndarray) -> list[dict]:
    logits_np = logits.detach().float().cpu().numpy().reshape(-1)
    projected = F.softplus(logits.detach().float()) + 1e-6
    projected = projected / projected.sum(dim=-1, keepdim=True)
    projected_np = projected.cpu().numpy().reshape(-1)
    ...
    stages = [
        ("raw_policy_shifted", raw_weights),
        ("softplus_projection", projected_np),
